...
router.post('/register', [body('username').trim().notEmpty(), body('email').isEmail().normalizeEmail(), body('password').isLength({ min: 8 }).withMessage('Use at least 8 characters.').matches(/[^a-zA-Z0-9\s]/).withMessage('Include one special character.'), validate], async (req, res, next) => {
  try {
