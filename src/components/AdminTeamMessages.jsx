...
export default function AdminTeamMessages({ previewMessages }) {
  ...
  useMessageDeletion(useCallback(id => { deletedIds.current.add(id); setMessages(items => items.filter(item => String(item.id || item._id) !== id)) }, []))
  ...
  const load = async () => {
    ...
    try { const result = await apiClient.getAdminTeamMessages(250); setMessages((result.messages || []).filter(item => !deletedIds.current.has(String(item.id || item._id)))); setPage(0) }
    catch (err) { setError(err.error || err.message || 'Could not load team messages.') }
    finally { setLoading(false) }
  }
  useEffect(() => { if (previewMessages) return; let alive = true; apiClient.getAdminTeamMessages(250).then(result => { if (alive) setMessages((result.messages || []).filter(item => !deletedIds.current.has(String(item.id || item._id)))) }).catch(err => { if (alive) setError(err.error || err.message || 'Could not load team messages.') }).finally(() => { if (alive) setLoading(false) }); return () => { alive = false } }, [previewMessages])
