...
class APIClient {
  ...
  async getSystemStatus() { try { return (await axios.get(`${API_BASE_URL}/health`, { timeout: 10000 })).data; } catch { return { atlas: { configured: true, connected: false } }; } }
  async getNotifications() { try { return (await axios.get(`${API_BASE_URL}/notifications`, { headers: this.getHeaders(), timeout: 10000 })).data; } catch (error) { throw error.response?.data || error; } }
